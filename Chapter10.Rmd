...
4. Comput a histogram of carat by "hand" using a binwidth of 0.1. Display the results with `geom_bar(stat = "identity")`. (Hint: you might need to create a new variable first.)
...
One theory: The batter's performance improves with more experience (higher number of at bats).

Second theory: According to the Central Limit Theorem, the batting average converges to a normal distribution as the number of at bats increases. When the number of at bats is small, the distribution may be skewed to the right (with the mass of the distribution pulling the points to the left).
...
```{r}
Batting %>% select(playerID) %>% unique() %>% count()
```

#TODO: Debug following. Remove NAs. Find max(lb).

```{r}
baPerBatter <- Batting %>% 
  mutate(baPerYear=H/AB) %>% 
  group_by(playerID) %>%
  summarize(totalHits=sum(H, na.rm=TRUE),totalAB=sum(AB, na.rm=TRUE),ba=sum(H, na.rm=TRUE)/sum(AB, na.rm=TRUE),n=n(),lq=quantile(baPerYear,0.25),uq=quantile(baPerYear,0.75)) %>%
  mutate(lb=meanba-se) %>% 
  filter(n>1,totalHits>n) #ignore players with too few hits (either b/c of few AB or few hits. Also ignore players who have only played for one year.)
  

head(baPerBatter)

ggplot(baPerBatter, aes(playerID,ba)) + geom_point()
```
