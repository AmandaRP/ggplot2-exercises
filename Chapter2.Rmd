...
We see that Toyota has the most models. Below we find that the Caravan 2wd has the most variations.
...
mpg2 %>% group_by(modelName) %>% unique() %>% count() -> variations
variations[which.max(variations$n),"modelName"]
```

Our new answer shows a4 as having the most variations.
...
Ans: It might be nice to order by the average engine size. The following code finds the average engine size per class, but I'm not sure how use it to sort the order of the boxes.

```{r}
avgDisplByClass <- mpg %>% group_by(class) %>% summarize(avgDispl=mean(displ))
avgDisplByClass
```
