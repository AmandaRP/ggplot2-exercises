...
```{r}
modelCount <- mpg[,c("manufacturer","model")] %>% group_by(manufacturer) %>% unique() %>% count()  
...
```{r}
variations <- mpg[,c("manufacturer", "model","cyl", "trans", "drv", "fl", "class")] %>% group_by(model) %>% unique() %>% count() 
